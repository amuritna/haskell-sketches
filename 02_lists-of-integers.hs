-- ga ngerti cara pakai lists di luar olympia jadi pasrah aja ye
-- ini cuma ngeramal doang, bakal gagal compile atau kagak aja ga tahu maaf

-----------------------------------------

-- COPAS DARI ListOfInteger.hs (diberikan saat praktikum)

-- DEFINISI DAN SPESIFIKASI LIST
{- type List of Char: [ ] atau [e o List] atau [List o e]
   Definisi type List of Char
   Basis: List of Char kosong adalah list of Char
   Rekurens: 
   List tidak kosong dibuat dengan menambahkan sebuah elemen bertype Char di awal 
   sebuah list atau
   dibuat dengan menambahkan sebuah elemen bertype Char di akhir sebuah list -}

-- DEFINISI DAN SPESIFIKASI KONSTRUKTOR
konso :: Char -> [Char] -> [Char]
{- konso e lc menghasilkan sebuah list of character dari e (sebuah character)  
   dan lc (list of integer), dengan cc sebagai elemen pertama: e o lc -> lc' -}
-- REALISASI
konso e lc = [e] ++ lc

konsDot :: [Char] -> Char -> [Char]
{- konsDot(lc,cc) menghasilkan sebuah list of character dari lc (list of 
   character) dan e (sebuah character), dengan e sebagai elemen terakhir: 
   lc o e -> lc' -}
-- REALISASI
konsDot lc e = lc ++ [e]

-- DEFINISI DAN SPESIFIKASI SELEKTOR
-- head :: [Char] -> Char
-- head l menghasilkan elemen pertama list l, l tidak kosong

-- tail :: [Char] -> [Char]
-- tail l menghasilkan list tanpa elemen pertama list l, l tidak kosong

-- last :: [Char] -> Char
-- last l  menghasilkan elemen terakhir list l, l tidak kosong

-- init :: [Char] -> [Char]
-- init(l) menghasilkan list tanpa elemen terakhir list l, l tidak kosong

-- DEFINISI DAN SPESIFIKASI PREDIKAT
isEmpty :: [Char] -> Bool
-- isEmpty l true jika list of character l kosong
-- REALISASI
isEmpty l = null l

isOneElmt :: [Char] -> Bool
-- isOneElmt l true jika list of character l hanya mempunyai satu elemen
-- REALISASI
isOneElmt l = (length l) == 1 
           
           
-------------------------------------------------------------------------

-- GET LOWEST
-- DEFINISI DAN SPESIFIKASI
lowest :: [Int] -> Int
{- Fungsi yang mencari integer yang paling kecil dari suatu list. -}

-- REALISASI
lowest l
    | isOneElmt l = head l --basis
    | otherwise = --rekurens
        if (head l) < (lowest (tail l)) then
            head l
        else
            lowest (tail l)

-- APLIKASI
-- lowest [3, 1, 4, 1, 5]
-- 1
-- lowest [4, 2, 0]
-- 0


          
-- DELETE ELEMENT
-- DEFINISI DAN SPESIFIKASI
delElmt :: [Int] -> [Int]
{- Diberikan input list L dan integer N, mengembalikan list L tanpa integer N. -}

-- REALISASI
delElmt lst n
	| isEmpty lst = [] --basis
	| isOneElmt lst = --basis
		if (head lst) == n then []
		else lst
	| otherwise = konso (delElmt (head lst) n) (delElmt (tail lst) n)
	
-- APLIKASI
-- delElmt [3, 1, 4, 1, 5] 1
-- [3, 4, 5]
-- delElmt [1, 1, 1, 1, 1] 1
-- []



-- SORT LIST OF INTEGERS
-- DEFINISI DAN SPESIFIKASI
sortInts :: [Int] -> [Int]
{- Mengurutkan lists of integers secara ascending. -}

-- REALISASI
sortInts l
	| (isEmpty l || isOneElmt l) = l
	| otherwise = [lowest l] ++ sortInts (delElmt (lowest l) l)
	
	
	
-- IS ALL GENAP
-- DEFINISI DAN SPESIFIKASI
allEven :: [Int] -> Bool
{- Mencari tahu apakah semua elemen di list of integers tersebut genap. -}

-- REALISASI
allEven l = ((mod (head l) 2) == 0) && (allEven (tail l))

-- APLIKASI
-- allEven [2, 4, 6, 8]
-- True
-- allEven [1, 2, 4, 6]
-- False
